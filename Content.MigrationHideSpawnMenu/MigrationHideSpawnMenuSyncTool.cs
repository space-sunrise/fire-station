    ...
    public static class MigrationHideSpawnMenuSyncTool
    {
            ...
            string editComment)
        {
            ...
            foreach (var prototypePath in Directory.EnumerateFiles(prototypesRoot, "*.yml", SearchOption.AllDirectories))
            {
                ...
                var parsed = MigrationHideSpawnMenuPrototypeParser.Parse(prototypePath, originalContent);
        ...
        {
            Console.WriteLine(
                $"mode={mode} filesScanned={summary.FilesScanned} filesChanged={summary.FilesChanged} " +
                $"candidatesFound={summary.CandidatesFound} candidatesUpdated={summary.CandidatesUpdated}");
        }
