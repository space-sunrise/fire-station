...
namespace Content.MigrationHideSpawnMenu
{
    internal static class MigrationHideSpawnMenuPrototypeParser
    {
        ...
        public static MigrationHideSpawnMenuPrototypeFile Parse(string filePath, string content)
        {
            ...
            var result = new MigrationHideSpawnMenuPrototypeFile(filePath, newLine, endsWithTrailingNewLine, lines);
        ...
        {
            if (categoriesRemainder.StartsWith("[", StringComparison.Ordinal))
            {
            ...
            for (var i = categoriesLineIndex + 1; i < endLineExclusive; i++)
            {
                ...
                if (indent == fieldIndent && !trimmed.StartsWith("-", StringComparison.Ordinal))
                    break;
        ...
        private readonly struct PrototypeEntry
        {
            public PrototypeEntry(int lineIndex, int indent, string prototypeType)
            {
                LineIndex = lineIndex;
                Indent = indent;
            ...
            public string PrototypeType { get; }
        }
    }
}
