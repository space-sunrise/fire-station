scp173-in-safe-time = Вы не можете засорять свою камеру еще {$mm}:{$ss}.
    Подождите и дайте сотрудникам Фонда время на подготовку к началу работы.
scp173-fast-movement-too-many-watchers = На вас смотрит слишком много людей!
scp173-damage-structures-blocked = Ваша камера содержания подавляет вашу способность к разрушению!
scp173-blind-failed-not-in-chamber = Вы должны находиться в камере содержания
scp173-blind-failed-too-few-watchers = Слишком мало смотрящих
scp-cage-suppress-ability = { $container } подавляет ваши аномальные способности!
scp173-cage-cannot-be-closed = Загрязненность камеры содержания мешает заслону закрыться
reagent-name-scp173 = коричневое нечто
reagent-desc-scp173 = отвратительно пахнущее вещество
scp173-reagent-bar-title = Загрязненность: { $current } из { $max }
scp173-safe-time-title = Безопасное время {$mm}:{$ss}
