...
{
    private static readonly Regex FaxLabelRegex = new(@"[*?!'%\s]");
    ...
    public void UpdateState(FaxUiState state)
    {
        ...
        //Sunrise-Start
        var localeKey = GetFaxLabelKey(state.DeviceName);
        if (Loc.GetString(localeKey) == localeKey)
        {
            FromLabel.Text = Loc.GetString(state.DeviceName); // Fire edit
        }
        else
        {
            FromLabel.Text = Loc.GetString(localeKey);
        }
        //Sunrise-End
    ...
    {
        //Sunrise-Start
        var localeKey = GetFaxLabelKey(name);
        var loc = name;
        if (Loc.GetString(localeKey) != localeKey)
        {
            loc = Loc.GetString(localeKey);
        }
    ...
    private static string GetFaxLabelKey(string name)
    {
        return $"fax-label-{FaxLabelRegex.Replace(name, string.Empty).ToLower()}";
    }
}
